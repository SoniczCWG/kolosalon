...
def gen_html(var_name,action,dict_like_temp,fused_temp=""):
    ...
    if "input" in fused_temp and "radio" in fused_temp:
        return "{input_form(\""+dict_like_temp["type"]+"\", \""+var_name+"\" ,objednavka."+var_name+""", (e) => {
            dispatch({
              type: "update_number",
    ...
    if "select" in fused_temp:
        a="{select_form(\"select1\",\"abc\",objekty[0].items)}"
    ...
    if "input" in fused_temp and "radio" not in fused_temp:            
        ...
        if dict_like_temp["type"] == "number":
            ...
            return """<input type="checkbox" id=\""""+var_name+"""\" onChange={(e) => {
              dispatch({
                type: \""""+action+"""\",
              });
            }} />"""
...
must_format = """[
    ...
    {obj_id: 3, tag:"input",  type:"checkbox"}, input_checkbox1 
...
print("const defaultObjednavka="+repr(defaultObjednavka)+"\n\n")
